...
async function fillTableAcessorios() {
  ...
  if (!response.ok) {
    Swal.fire({
      icon: "error",
      text: "não foi possivel buscar dados na base",
    });
  } else {
...
window.deleteRow = async function (button) {
  try {
    ...
    Swal.fire({
      icon: "error",
      title: "ERRO",
      title: "Não foi possivel remover item...",
    });
  }
};
...
function insertButtonCellTable() {
  return `
        <td style="text-align: center;">
            <button class="btn btn-danger" type="button" style="padding: 0px;margin-left: 10px;" onclick="deleteRow(this)">
...
addEventById("#bt_adicionar", "click", insertAcessorios);

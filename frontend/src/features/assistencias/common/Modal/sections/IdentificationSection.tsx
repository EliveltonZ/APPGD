    ...
    <div className="ap-section">
      ...
      <div className="ap-section__body">
        <div className="frow--4">
          ...
          <SectionField label="Corte"  value={data.corte}  onChange={(v) => onChange('corte', v)}  readOnly={readOnly} />
          <SectionField label="Pedido" value={data.pedido} onChange={(v) => onChange('pedido', v)} readOnly={readOnly} />
        </div>
        ...
        <div>
          ...
          <div style={{ paddingTop: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {data.urgente === 'sim' && <span className="ap-urgente-tag">Urgente</span>}
          </div>
        </div>
      </div>
    </div>

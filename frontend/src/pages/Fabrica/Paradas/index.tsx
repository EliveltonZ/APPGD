...
const CAMPO_LABEL: Record<string, string> = {
  pedido: 'Pedido', data_inicio: 'Início', data_fim: 'Fim', id_maquina: 'Máquina',
};
...
export function ParadasPage() {
  ...
  const carregarBase = useCallback(async () => {
    const promises: [Promise<Maquina[]>, Promise<TipoReq[]>] = [fetchMaquinas(), fetchTipos()];
    const [m, t] = await Promise.all(promises);
    setMaquinas(m); setTipos(t);
  }, []);
  ...
  async function handlePedidoBlur() {
    setPedidoInfo(null); setPedidoErr('');
    const num = pedidoInput.trim();
    if (!tipoId || !num) return;
    if (isNaN(Number(num))) { setPedidoErr('Pedido inválido'); return; }
  ...
  async function handleMaquinaChange(id: string) {
    ...
    setMaquinaId(id); setParadaAberta(null);
    if (!id) return;
  ...
  function handleTipoChange(id: string) {
    if (maquinaBloqueada) return;
    setTipoId(id); setPedidoInput(''); setPedidoInfo(null); setPedidoErr('');
  }

  const formBloqueado  = maquinaBloqueada !== '';
  const formValido     = tipoId !== '' && pedidoInput.trim() !== '' && pedidoInfo !== null && maquinaId !== '';
  const podeIniciar    = formValido && !paradaAberta && !formBloqueado;
  const podeFinalizar  = maquinaId !== '' && paradaAberta !== null &&
  ...
  async function confirmarIniciar() {
    ...
    setSaving(true);
    try {
      await iniciarParada({ pedido: Number(pedidoInput), id_maquina: Number(maquinaId), id_tipo: Number(tipoId) });
    ...
    fetchParadaAberta(Number(maquinaId)).then(setParadaAberta).catch(() => {});
  ...
  function abrirEdicao(p: ParadaRow) {
    setEditando(p);
    setEditForm({ pedido: p.pedido, data_inicio: p.data_inicio ?? undefined, data_fim: p.data_fim ?? undefined, id_maquina: p.id_maquina });
  }
  ...
  // ── Labels ───────────────────────────────────────────────────────────────
  const nomeMaquina     = maquinas.find(m => String(m.id) === maquinaId)?.nome ?? '';
  const nomeMaqBloqueada = maquinas.find(m => String(m.id) === maquinaBloqueada)?.nome ?? '';
  ...
  return (
    <AppLayout title="Paradas de Máquina">
      <div className="paradas-layout">
        ...
        {formBloqueado && (
          <div className="paradas-aviso">
            ...
            Parada ativa em <strong>{nomeMaqBloqueada}</strong> — finalize antes de iniciar outra.
          </div>
        )}
          ...
          <div className="paradas-card">
            ...
            <div className="paradas-form">
              <div className="paradas-field">
                ...
                <select value={tipoId} onChange={e => handleTipoChange(e.target.value)} disabled={formBloqueado}>
              ...
              <div className="paradas-field">
                <label>Pedido *</label>
                <input
                  type="text" value={pedidoInput} placeholder="Nº do pedido"
                  disabled={!tipoId || formBloqueado}
                  className={formBloqueado ? 'paradas-field__readonly' : ''}
                  onChange={e => { if (!formBloqueado) { setPedidoInput(e.target.value); setPedidoInfo(null); setPedidoErr(''); } }}
                  onBlur={handlePedidoBlur}
                />
                {pedidoErr && <span className="paradas-field__error">{pedidoErr}</span>}
              </div>
              <div className="paradas-field">
                <label>Máquina *</label>
                <select value={maquinaId} onChange={e => handleMaquinaChange(e.target.value)}
                  disabled={formBloqueado} className={formBloqueado ? 'paradas-field__readonly' : ''}>
                  <option value="">Selecione...</option>
              ...
              <div className="paradas-fields-row">
                <div className="paradas-field">
                  <label>Cliente</label>
                  <input type="text" value={pedidoInfo?.cliente ?? ''} readOnly className="paradas-field__readonly" />
                </div>
                <div className="paradas-field">
                  <label>Ambiente</label>
                  <input type="text" value={pedidoInfo?.ambiente ?? ''} readOnly className="paradas-field__readonly" />
                </div>
              </div>
              {maquinaId && (
                <div className={`paradas-status ${paradaAberta ? 'paradas-status--em-parada' : 'paradas-status--disponivel'}`}>
                  {paradaAberta
            ...
            <div className="paradas-actions">
              <button className="paradas-btn paradas-btn--iniciar"
                onClick={() => setPendingAction('iniciar')} disabled={!podeIniciar || saving}>
                Iniciar Parada
              </button>
              <button className="paradas-btn paradas-btn--finalizar"
                onClick={() => setPendingAction('finalizar')} disabled={!podeFinalizar || saving}>
        ...
        <div className="paradas-card">
          <div className="paradas-table-header">
            <h2 className="paradas-card__title" style={{ margin: 0 }}>Histórico de Paradas</h2>
          ...
          {paradas.length === 0 ? (
            ...
            <div className="paradas-table-wrap">
              <table className="paradas-table">
                <thead>
                  <tr>
                    <th>#</th><th>Tipo</th><th>Pedido</th><th>Máquina</th>
                ...
                <tbody>
                  {paradas.map(p => (
                    <tr key={p.id} className={!p.data_fim ? 'paradas-table__row--aberta' : ''}>
                      ...
                      <td>{p.data_fim ? fmtDt(p.data_fim) : <span className="paradas-badge--aberta">Em aberto</span>}</td>
      ...
      {pendingAction && (
        <div className="paradas-modal-overlay" onClick={() => setPendingAction(null)}>
          <div className="paradas-modal paradas-confirm" onClick={e => e.stopPropagation()}>
            {pendingAction === 'iniciar' ? (
              <>
                <div className="paradas-confirm__icon paradas-confirm__icon--iniciar">▶</div>
                <h3 className="paradas-confirm__title">Confirmar Início de Parada</h3>
                <div className="paradas-confirm__details">
                  <div className="paradas-confirm__row"><span>Máquina</span><strong>{nomeMaquina}</strong></div>
                  <div className="paradas-confirm__row"><span>Tipo</span><strong>{nomeTipo}</strong></div>
                  <div className="paradas-confirm__row"><span>Pedido</span><strong>{pedidoInput}</strong></div>
                  {pedidoInfo?.cliente  && <div className="paradas-confirm__row"><span>Cliente</span><strong>{pedidoInfo.cliente}</strong></div>}
                  {pedidoInfo?.ambiente && <div className="paradas-confirm__row"><span>Ambiente</span><strong>{pedidoInfo.ambiente}</strong></div>}
                </div>
                <p className="paradas-confirm__msg">Deseja registrar o início desta parada?</p>
              </>
            ) : (
              <>
                <div className="paradas-confirm__icon paradas-confirm__icon--finalizar">■</div>
                <h3 className="paradas-confirm__title">Confirmar Fim de Parada</h3>
                <div className="paradas-confirm__details">
                  <div className="paradas-confirm__row"><span>Máquina</span><strong>{nomeMaquina}</strong></div>
                  {paradaAberta && <>
                    <div className="paradas-confirm__row"><span>Pedido</span><strong>{paradaAberta.pedido}</strong></div>
                    <div className="paradas-confirm__row"><span>Desde</span><strong>{fmtDt(paradaAberta.data_inicio)}</strong></div>
                  </>}
                </div>
                <p className="paradas-confirm__msg">Deseja registrar o fim desta parada?</p>
              </>
            )}
            <div className="paradas-actions paradas-confirm__actions">
              ...
              <button className="paradas-btn" onClick={() => setPendingAction(null)}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="paradas-modal-overlay" onClick={() => setEditando(null)}>
          <div className="paradas-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="paradas-form">
              <div className="paradas-field">
                <label>Pedido</label>
                <input type="number" value={editForm.pedido ?? ''}
                  onChange={e => setEditForm(f => ({ ...f, pedido: Number(e.target.value) }))} />
              </div>
              <div className="paradas-field">
                <label>Máquina</label>
                <select value={editForm.id_maquina ?? ''}
                  onChange={e => setEditForm(f => ({ ...f, id_maquina: Number(e.target.value) }))}>
                  {maquinas.map(m => <option key={m.id} value={m.id}>{m.nome}</option>)}
                </select>
              </div>
              <div className="paradas-field">
                <label>Data/Hora Início</label>
                <input type="datetime-local" value={toInputDt(editForm.data_inicio)}
                  onChange={e => setEditForm(f => ({ ...f, data_inicio: e.target.value ? new Date(e.target.value).toISOString() : null }))} />
              </div>
              <div className="paradas-field">
                <label>Data/Hora Fim</label>
                <input type="datetime-local" value={toInputDt(editForm.data_fim)}
                  onChange={e => setEditForm(f => ({ ...f, data_fim: e.target.value ? new Date(e.target.value).toISOString() : null }))} />
              </div>
            </div>
            <div className="paradas-actions">
              <button className="paradas-btn paradas-btn--finalizar" onClick={salvarEdicao} disabled={editSaving}>
